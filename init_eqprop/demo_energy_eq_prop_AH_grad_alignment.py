...
from artemis.plotting.db_plotting import dbplot, DBPlotTypes
from artemis.plotting.expanding_subplots import add_subplot, vstack_plots
...
def cosine_similarity(a, b, axis):
    return (a * b).sum(axis=axis) / tt.sqrt((a ** 2).sum(axis=axis) * (b ** 2).sum(axis=axis))


@symbolic
def compute_gradient_alignment(forward_params, eq_states, x, nonlinearity):

    def get_grads(disconnect):
        forward_states = forward_pass(params=forward_params, x=x, nonlinearity=nonlinearity, disconnect_grads=disconnect)
        loss = sum((.5 * (fs - es) ** 2).sum(axis=1) for fs, es in izip_equal(forward_states, eq_states)).mean(axis=0)
        weight_params = [w for w, b in forward_params]
        flat_param_grads = tt.grad(loss, wrt=weight_params)
        return flat_param_grads

    local_grads = get_grads(disconnect=True)
    global_grads = get_grads(disconnect=False)

    cosine_sim = [cosine_similarity(lg.flatten(), (gg-lg).flatten(), axis=0) for lg, gg in izip_equal(local_grads, global_grads)]
    return cosine_sim
...
def demo_energy_based_initialize_eq_prop_alignment(
    ...
    f_grad_align = compute_gradient_alignment.partial(nonlinearity=forward_nonlinearity).compile()
    ...
    for i, (ixs, info) in enumerate(minibatch_index_info_generator(n_samples=x_train.shape[0], minibatch_size=minibatch_size, n_epochs=n_epochs)):
        ...
        if is_epoch_checkpoint(epoch):
            ...
            # ===== Compute alignment
            alignment_neg_states = forward_states_ = f_forward_pass(x=x_train, params=forward_params) if train_with_forward else initialize_states(n_samples=x_train.shape[0], noninput_layer_sizes=layer_sizes[1:])
            for _ in range(n_negative_steps):
                alignment_neg_states = f_inference_eq_step(params=eq_params, states = alignment_neg_states, x=x_train, epsilon=epsilon)

            grad_alignments = f_grad_align(forward_params=forward_params, eq_states=alignment_neg_states, x=x_train)
            dbplot(grad_alignments, 'alignments', plot_type=DBPlotTypes.LINE_HISTORY)
            # --------------------
            ...
            results[next, :] = dict(iter=i, epoch=epoch, test_init_error=test_init_error, test_neg_error=test_neg_error, train_init_error=train_init_error, train_neg_error=train_neg_error, alignments=grad_alignments)
            yield results
        ...
        # if i%100==0:
        #     grad_alignments = f_grad_align(forward_params=forward_params, eq_states=negative_states, x=x_data_sample)
        #     dbplot(grad_alignments, 'alignments', plot_type=DBPlotTypes.LINE_HISTORY)
# ======================================================================================================================

baseline_losstype = demo_energy_based_initialize_eq_prop_alignment.add_root_variant('losstype_baseline')
...
with capture_created_experiments() as exps_reduced_steps:
    X = demo_energy_based_initialize_eq_prop_alignment.add_root_variant('stepsize_baseline', random_flip_beta=True)
...
with capture_created_experiments() as deeper_experiments:
    X = demo_energy_based_initialize_eq_prop_alignment.add_root_variant('large',
...
with capture_created_experiments() as local_loss_exp:
    X = demo_energy_based_initialize_eq_prop_alignment.add_root_variant('local_loss_baseline', train_with_forward='contrast', random_flip_beta=False, forward_nonlinearity='rho(x)+0.01*x')
    X.add_variant(local_loss = False)
    X.add_variant(local_loss = True)
    X = demo_energy_based_initialize_eq_prop_alignment.add_root_variant('local_loss_baseline_energy', train_with_forward='energy', random_flip_beta=False, forward_nonlinearity='rho(x)+0.01*x')
    X.add_variant(local_loss = False)
    X.add_variant(local_loss = True)
...
with capture_created_experiments() as exps_minimal_neg_convergence:
    X = demo_energy_based_initialize_eq_prop_alignment.add_root_variant('min_neg_baseline', train_with_forward='contrast+', forward_nonlinearity='rho(x)+0.01*x')
    for random_flip_beta in (False, True):
...
baseline_pos_or_neg = demo_energy_based_initialize_eq_prop_alignment.add_root_variant('pos_or_neg', random_flip_beta=True, forward_nonlinearity='rho(x)+0.01*x')
...
def show_alignment_plot(record):
    ...
    with vstack_plots(grid=True, spacing=0.1, bottom_pad=0.1, left_pad=0.1):
        ...
        add_subplot()
        plt.plot(duck[:, 'epoch'], duck[:, 'test_init_error'], label='Init Eq. Prop: $s^f$', color='C2')
        plt.plot(duck[:, 'epoch'], duck[:, 'test_neg_error'], label='Init Eq. Prop: $s^-$', color='C1')
        ...
        for i, (alignments, c) in enumerate(zip(alignments[:-1], colors)):
            plt.plot(duck[:, 'epoch'], alignments, label=f'$S(\\nabla_{{\phi_{i+1}}} L_{i+1}, \\nabla_{{\phi_{i+1}}} L_{{{i+2}:{n_layers}}})$', color=c)
...
ex_large_alignment = demo_energy_based_initialize_eq_prop_alignment.get_variant('large').add_variant(train_with_forward='contrast', n_negative_steps=50, forward_nonlinearity='rho(x)+0.01*x', skip_zero_epoch_test=False)
ex_large_alignment.add_variant(local_loss = False)
...
if __name__ == '__main__':
    ...
    # ex = demo_energy_based_initialize_eq_prop_alignment.get_variant('large').add_variant(train_with_forward='contrast', n_negative_steps=50, forward_nonlinearity='rho(x)+0.01*x', skip_zero_epoch_test=False)
    # ex.run()

    ex_large_alignment.browse()

    # record = ex_large_alignment.get_latest_record()
    # show_alignment_plot(record)
